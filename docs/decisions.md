...
| D23 | **Every store's Stripe account asks for cards, Klarna, Link and MobilePay.** Kaizen requests these payment capabilities when it creates a store's account (test and live), and for older accounts in the background when the store's admin is opened or a checkout starts; it records what it asked for (`payment_methods_requested`). Apple Pay and Google Pay come with cards. | 2026-09-24 | Stripe then offers each method where it fits the shopper's country and currency (MobilePay in Denmark and Finland), and the store's own Stripe settings still decide. Swish and Vipps are in private preview at Stripe: Kaizen's platform account must be given access first, and Vipps is not yet available for Accounts v2; they are added to the list once possible. PayPal is not offered: Stripe does not allow it for platforms whose stores take direct charges on their own accounts. |
