-- Pages and articles (D57) each have their own addresses, and so do their
-- redirects: a page taking an address replaces a redirect of its own type,
-- and a published page or article moving leaves one of its type.
CREATE OR REPLACE FUNCTION commerce.pages_keep_addresses() RETURNS trigger
LANGUAGE plpgsql SET search_path = '' AS $$
BEGIN
  DELETE FROM commerce.page_redirects r
  WHERE r.slug = NEW.slug AND r.type = NEW.type AND r.store_id IS NOT DISTINCT FROM NEW.store_id;
  IF TG_OP = 'UPDATE' AND NEW.slug <> OLD.slug AND OLD.published_at IS NOT NULL THEN
    INSERT INTO commerce.page_redirects (store_id, type, slug, page_id)
    VALUES (NEW.store_id, NEW.type, OLD.slug, NEW.id)
...
-- A redirect takes its page's type, and never shadows an address of that type in use.
CREATE OR REPLACE FUNCTION commerce.page_redirects_not_taken() RETURNS trigger
LANGUAGE plpgsql SET search_path = '' AS $$
BEGIN
  SELECT p.type INTO NEW.type FROM commerce.pages p WHERE p.id = NEW.page_id;
  IF EXISTS (
    SELECT 1 FROM commerce.pages p
    WHERE p.slug = NEW.slug AND p.type = NEW.type AND p.store_id IS NOT DISTINCT FROM NEW.store_id
  ) THEN
...
$$;
--> statement-breakpoint
-- A store's front page (D54) is a page, not an article.
CREATE OR REPLACE FUNCTION commerce.stores_front_page_is_page() RETURNS trigger
LANGUAGE plpgsql SET search_path = '' AS $$
BEGIN
  IF NEW.front_page_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM commerce.pages p WHERE p.id = NEW.front_page_id AND p.type <> 'page'
  ) THEN
    RAISE EXCEPTION 'a front page must be a page, not an article';
  END IF;
  RETURN NEW;
END;
$$;
--> statement-breakpoint
CREATE TRIGGER stores_front_page_is_page
BEFORE INSERT OR UPDATE OF front_page_id ON commerce.stores
FOR EACH ROW EXECUTE FUNCTION commerce.stores_front_page_is_page();
--> statement-breakpoint
-- Pages published before the first date was kept count from their last publishing.
UPDATE commerce.pages SET first_published_at = published_at WHERE published_at IS NOT NULL AND first_published_at IS NULL;
--> statement-breakpoint
-- New stores copy the template's articles as articles (D57).
