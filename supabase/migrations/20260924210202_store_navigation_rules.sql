-- D30: the storefront's logo and menus.

-- An object: {"logo": …, "header": […], "footer": […]}; its shape is checked by the app.
ALTER TABLE commerce.stores ADD CONSTRAINT stores_navigation_object
  CHECK (jsonb_typeof(navigation) = 'object');
--> statement-breakpoint

-- New stores start with the template's logo and menus. Menu links name
-- products by handle, which the copies keep, so they work unchanged.
