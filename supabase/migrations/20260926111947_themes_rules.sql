-- Saved themes (D60). Like every commerce table: row-level security on, no policies.
ALTER TABLE commerce.store_themes ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
-- Fonts chosen before themes (D59) become part of each store's theme.
UPDATE commerce.stores
   SET theme = jsonb_build_object('settings', jsonb_build_object('fonts', fonts))
 WHERE fonts <> '{}'::jsonb AND theme = '{}'::jsonb;
--> statement-breakpoint
-- The demo's product grid draws the theme's product cards (D60) rather than
-- square pictures of its own, so a theme changes the whole front page.
UPDATE commerce.pages p
   SET draft = CASE WHEN p.draft #>> '{rows,0,columns,0,blocks,1,id}' = 'front-products'
                    THEN p.draft #- '{rows,0,columns,0,blocks,1,imageShape}' ELSE p.draft END,
       published = CASE WHEN p.published #>> '{rows,0,columns,0,blocks,1,id}' = 'front-products'
                        THEN p.published #- '{rows,0,columns,0,blocks,1,imageShape}' ELSE p.published END
  FROM commerce.stores s
 WHERE s.id = p.store_id AND s.is_template AND p.slug = 'forside';
--> statement-breakpoint
-- New stores start with the template's theme (D60), fonts included, as with its menus.
...
BEGIN
  ...
  INSERT INTO commerce.stores (slug, name, created_by, navigation, fonts, theme)
  SELECT p_slug, p_name, p_owner_id, navigation, fonts, theme - 'savedId' FROM commerce.stores WHERE id = p_template_id
