-- Categories and tags (decision D50). Like every commerce table: row-level security on, no policies.
ALTER TABLE commerce.terms ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE commerce.product_terms ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
-- A category's parent is a category of the same owner and content, and a
-- category is never inside itself, however deep.
CREATE FUNCTION commerce.terms_parent_valid() RETURNS trigger
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM commerce.terms p
    WHERE p.id = NEW.parent_id
      AND p.kind = 'category'
      AND p.content_type = NEW.content_type
      AND p.store_id IS NOT DISTINCT FROM NEW.store_id
  ) THEN
    RAISE EXCEPTION 'a category''s parent is a category of the same kind of content' USING ERRCODE = 'check_violation';
  END IF;
  WHILE ancestor IS NOT NULL LOOP
    IF ancestor = NEW.id OR depth > 50 THEN
      RAISE EXCEPTION 'a category cannot be inside itself' USING ERRCODE = 'check_violation';
    END IF;
    SELECT t.parent_id INTO ancestor FROM commerce.terms t WHERE t.id = ancestor;
    depth := depth + 1;
  END LOOP;
  RETURN NEW;
END;
$$;
--> statement-breakpoint
CREATE TRIGGER terms_parent_valid
BEFORE INSERT OR UPDATE OF parent_id, store_id, content_type, kind ON commerce.terms
FOR EACH ROW EXECUTE FUNCTION commerce.terms_parent_valid();
--> statement-breakpoint
-- New stores start with copies of the template's product categories and
-- tags, and its products keep theirs. The rest is as before (D30).
