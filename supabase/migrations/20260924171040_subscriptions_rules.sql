-- Subscriptions as purchase options (decision D25): rules the schema file cannot express.

-- An order started or renewed a subscription of the same store. (The schema
-- file cannot declare it: subscriptions also point back at their first order.)
ALTER TABLE commerce.orders
  ADD CONSTRAINT orders_subscription_fk FOREIGN KEY (store_id, subscription_id)
  REFERENCES commerce.subscriptions (store_id, id);
--> statement-breakpoint

-- Like every commerce table: row-level security on, no policies.
ALTER TABLE commerce.selling_plans ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE commerce.subscriptions ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE commerce.subscription_lines ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint

-- New stores copy the template's purchase options with its products.
...
BEGIN
  ...
  INSERT INTO commerce.selling_plans (
    id, store_id, product_id, interval, interval_count, discount_percent, position, active
  )
  SELECT commerce.clone_id(v_store, sp.id), v_store, commerce.clone_id(v_store, sp.product_id),
         sp.interval, sp.interval_count, sp.discount_percent, sp.position, sp.active
