...
describe("saved rows, columns and components", () => {
  it("saves each kind, lists them, renames and changes one, and deletes it", async () => {
    const row = await saved.createSavedPart(admin, null, {
      kind: "row",
      name: `Hero ${run}`,
      content: { id: "r", type: "row", layout: "2", columns: [{ id: "c1", blocks: [text("b1", "Left")] }, { id: "c2", blocks: [] }] },
    });
    expect(row.ok).toBe(true);
    const column = await saved.createSavedPart(admin, null, { kind: "column", name: `Aside ${run}`, content: { id: "c", blocks: [] } });
    const block = await saved.createSavedPart(admin, null, { kind: "block", name: `Promise ${run}`, content: text("b", "Fast.") });
    if (!row.ok || !column.ok || !block.ok) throw new Error("not saved");

    const mine = (await saved.listSavedParts(null)).filter((p) => p.name.endsWith(run));
    ...
    const changed = await saved.updateSavedPart(admin, null, block.id, { kind: "block", name: `Promise v2 ${run}`, content: text("b", "Faster.") });
    ...
    expect(await saved.updateSavedPart(admin, null, block.id, { kind: "column", name: "x", content: { id: "c", blocks: [] } })).toEqual({
      ok: false,
      problems: ["This saved part no longer exists."],
    });

    await saved.deleteSavedPart(admin, null, column.id);
    expect((await saved.listSavedParts(null)).some((p) => p.id === column.id)).toBe(false);
  });

  it("refuses a nameless part, unsafe text and a row with the wrong columns", async () => {
    const results = await Promise.all([
      saved.createSavedPart(admin, null, { kind: "block", name: " ", content: text("b", "x") }),
      saved.createSavedPart(admin, null, {
        kind: "block",
        name: `Bad ${run}`,
        content: {
          id: "b",
      ...
      saved.createSavedPart(admin, null, { kind: "row", name: `Bad row ${run}`, content: { id: "r", type: "row", layout: "3", columns: [] } }),
    ]);
