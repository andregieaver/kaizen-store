...
export type EmailBlock =
  ...
  | { type: "lines"; rows: { label: string; value: string; strong?: boolean; muted?: boolean }[] }
  ...
  switch (block.type) {
    ...
    case "lines":
      return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 16px;border-collapse:collapse">${block.rows
        .map(
          (row) =>
            `<tr><td style="padding:6px 0;font-size:15px;color:${row.muted ? "#525252" : "#171717"};${row.strong ? "font-weight:700;" : ""}">${escapeHtml(row.label)}</td><td align="right" style="padding:6px 0 6px 12px;font-size:15px;white-space:nowrap;color:${row.muted ? "#525252" : "#171717"};${row.strong ? "font-weight:700;" : ""}">${escapeHtml(row.value)}</td></tr>`,
        )
        .join("")}</table>`;
  }
}
